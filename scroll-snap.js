...
var ScrollSnap = /** @class */ (function () {
  function ScrollSnap(element, config) {
    ...
    this.animationHandler = function () {
      ...
      if (
        _this.scrollStart.y === _this.target.scrollTop &&
    ...
    var timeout = config.timeout,
      ...
      snapDestinationX = config.snapDestinationX,
      snapDestinationY = config.snapDestinationY
    if (timeout && (isNaN(timeout) || typeof timeout === 'boolean')) {
      throw new Error(
        "Optional config property 'timeout' is not valid, expected NUMBER but found " +
          (typeof timeout).toUpperCase()
      )
    }
    ...
    if (duration && (isNaN(duration) || typeof duration === 'boolean')) {
      throw new Error(
        "Optional config property 'duration' is not valid, expected NUMBER but found " +
          (typeof duration).toUpperCase()
      )
    }
    this.duration = duration || DURATION_DEFAULT
    if (easing && typeof easing !== 'function') {
      throw new Error(
        "Optional config property 'easing' is not valid, expected FUNCTION but found " +
          (typeof easing).toUpperCase()
      )
    }
    this.easing = easing || EASING_DEFAULT
    if (snapDestinationX && typeof snapDestinationX !== 'string') {
      throw new Error(
        "Optional config property 'snapDestinationX' is not valid, expected STRING but found " +
          (typeof easing).toUpperCase()
      )
    }
    ...
    if (snapDestinationY && typeof snapDestinationY !== 'string') {
      throw new Error(
        "Optional config property 'snapDestinationY' is not valid, expected STRING but found " +
          (typeof easing).toUpperCase()
      )
    }
